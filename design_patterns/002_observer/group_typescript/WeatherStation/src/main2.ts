...
import { DisplayHeatIndex } from './DisplayHeatIndex';
...
function main(): void {
    ...
    const heatIndex = new DisplayHeatIndex(weatherData);
    ...
    try {
        ...
        weatherData.setMeasurements({ pressure: 45.0 });

        weatherData.unsubscribe(heatIndex);
        weatherData.setMeasurements({ temperature: 96, humidity: 89, pressure: 66.0 });
